...
/**
    ...
    public function workreport()
    {
		...
		$result = array(); // to include all lines to print
		
		//Workreports info
		$lines = TableRegistry::get('Workreports'); 
	    $query0 = $lines->find();		
		
		//Fincas info
		$report = TableRegistry::get('Fincas'); 
	
		foreach ( $query0 as $lines ){  //for every record in work reports , retrieve necessary info from Fincas..
			//debug($lines->finca_id); 
			
			$query = $report // Start a new query for fincas
				->find()  // I can do in a controller $query = $report->find();
				->select(['id', 'prov', 'municipio', 'paraje', 'poligono', 'parcela', 'f_ult_poda' , 'f_ult_recog' , 'zona_id', 'transportista_id', 'agricultore_id'])
				-> where(['id' => $lines->finca_id ]) //WHERE Workreports->finca_id = $Fincas->id 
				->order (['f_ult_recog' => 'DESC' ]);
		 		
			foreach ($query as $report) { //We take the necessary info from Workreports
				$report->id = $lines->id;
				$report->finca_id = $lines->finca_id;
				$report->f_ult_recog = $lines->fecha_ult_recog;
				$report->transportista_id = $lines->transportista_id;
				
				$result[] = $report; //add to container to print in the view			    
			}
		}
