...
<div class="fincas form large-10 medium-8 columns content">
    ...
    <fieldset>
        ...
        <?php
			...
			echo $this->Form->control('fcomprafinca', ['label' => 'Fecha de compra de la finca (opcional) ']);
            echo $this->Form->control('f_ult_poda', ['label' => 'Fecha de la última poda, cambiando este campo se solicita la recogida'], ['empty' => true] );
			echo $this->Form->control ('agricultore_id', ['label' => 'Agricultor_id'], ['default' => $this->request->session()->read('agricultore_id')] );
