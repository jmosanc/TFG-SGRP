...
/**
  ...
  */
$this->assign('title', 'Listado de Trabajo');
...
<div class="fincas index large-10 medium-8 columns content">
    <h3><?= __('Lista de trabajo') ?></h3>
    <table cellpadding="0" cellspacing="0">
        <thead>
            <tr>
                <th width="10px" scope="col"><?= $this->Paginator->sort('id') ?></th>
            ...
            <tr>
				...
				<td><?= h($finca->agricultore_id) ?></td>
   			    <td class="actions">
					...
					if ( $this->request->session()->read('user_role') == 'a' or $this->request->session()->read('user_role') == 't' ) { //only if 'admin' or 'transportista' 	 ?>
                      <?= $this->Form->postLink(__('Hecha'), ['action' => 'complete', $finca->id], ['confirm' => __('¿Desea marcar como finca recogida? {0}?', $finca->id)]); 
					} ?>
